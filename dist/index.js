...
function hasAnyOptions(options) {
    return (options.chapterLevel !== undefined ||
...
function register(registry, options = {}) {
    registry.postprocessor(function () {
        this.process(function (document, output) {
            const pluginEnabled = hasAnyOptions(options) || hasAnyHeaderAttribute(document);
            if (!pluginEnabled) {
                return output;
            }
            const chapterLevel = toValidChapterLevel(firstDefined(options.chapterLevel, document.getAttribute(ATTRIBUTE_NAMES.chapterLevel), 1), 1);
            const labels = {
                image: firstDefined(options.labels?.image, document.getAttribute(ATTRIBUTE_NAMES.labels.image), DEFAULT_LABELS.image) ?? DEFAULT_LABELS.image,
                table: firstDefined(options.labels?.table, document.getAttribute(ATTRIBUTE_NAMES.labels.table), DEFAULT_LABELS.table) ?? DEFAULT_LABELS.table,
                stem: firstDefined(options.labels?.stem, document.getAttribute(ATTRIBUTE_NAMES.labels.stem), DEFAULT_LABELS.stem) ?? DEFAULT_LABELS.stem
            };
            ...
            for (let i = 0; i < lines.length; i += 1) {
                ...
                if (blockContext === 'image' && line.includes('<div class="title">')) {
                    ...
                    lines[i] = line.replace(/<div class="title">.*?\.\s*/, `<div class="title">${labels.image} ${effectiveChapter}-${counters.image}. `);
                    blockContext = null;
                    continue;
                }
                if (blockContext === 'table' &&
                    line.includes('<caption class="title">')) {
                    counters.table += 1;
                    const effectiveChapter = chapter || 1;
                    lines[i] = line.replace(/<caption class="title">.*?\.\s*/, `<caption class="title">${labels.table} ${effectiveChapter}-${counters.table}. `);
                    blockContext = null;
                    continue;
                }
                if (blockContext === 'stem' && line.includes('<div class="title">')) {
                    counters.stem += 1;
                    const effectiveChapter = chapter || 1;
                    lines[i] = line.replace(/<div class="title">/, `<div class="title">${labels.stem} ${effectiveChapter}-${counters.stem}. `);
                    blockContext = null;
                    continue;
                }
            }
